...
public class Database {
  ...
  Map<String, Integer> getFrequencyMap() throws SQLException {
    ...
    PreparedStatement prep = conn.prepareStatement(
            "SELECT corpus.filename, COUNT(*) " +
                    "FROM corpus JOIN word ON corpus.id=word.corpus_id " +
                    "GROUP BY corpus.id;"); //Your SQL here!
    ResultSet rs = prep.executeQuery();
    while (rs.next()) {
  ...
  Map<String, Integer> getInstanceMap() throws SQLException {
    ...
    PreparedStatement prep = conn.prepareStatement(
            "SELECT word, COUNT(*) FROM word ORDER BY COUNT(*) DESC LIMIT 5;"); //Your SQL Here!
    ResultSet rs = prep.executeQuery();
    while (rs.next()) {
